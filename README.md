# Vision Lab Setup

Setup guides for Vision Lab members to configure their computing environments.

**Already set up?** Check the [Changelog](docs/changelog.md) for updates you may need to apply.

## Getting Started

| Task                           | Guide                                                            | Status  |
| ------------------------------ | ---------------------------------------------------------------- | ------- |
| 1. Project Management Overview | [docs/project-management.md](docs/project-management.md)         | Draft   |
| 2. Create your user accounts   | [docs/create-accounts.md](docs/create-accounts.md)               | Draft   |
| 3. Configure Laptop            | [docs/laptop-macos.md](docs/laptop-macos.md)                     | Draft   |
| 4. Get Cluster Access          | [docs/getting-cluster-access.md](docs/getting-cluster-access.md) | Draft   |
| 5. Configure Cluster           | [docs/harvard-cluster.md](docs/harvard-cluster.md)               | Draft   |
| 6. Lightning AI                | docs/lightning-ai.md                                             | Planned |
| 7. Lab Workstations            | docs/lab-workstations.md                                         | Planned |

## Advanced Configurations

| Task                 | Guide                                                    | Status  |
| -------------------- | -------------------------------------------------------- | ------- |
| Terminal & SSH Setup | [docs/terminal-ssh-setup.md](docs/terminal-ssh-setup.md) | Draft   |
| Using the Cluster    | docs/cluster-usage.md                                    | Planned |

## Project Management, Project Templates, and Workflows

| Task                       | Guide                                                                                       | Status  |
| -------------------------- | ------------------------------------------------------------------------------------------- | ------- |
| Visionlab Project Template | [harvard-visionlab/project-template](https://github.com/harvard-visionlab/project-template) | Draft   |
| Project Demo               | [harvard-visionlab/project-demo](https://github.com/harvard-visionlab/project-demo)         | in progress   |
| Launching SLURM JOBS       | docs/slurm.md                                                                               | Planned |
| Model Training             | docs/model-training.md                                                                      | Planned |
